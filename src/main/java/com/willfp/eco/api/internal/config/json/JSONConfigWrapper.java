...
public abstract class JSONConfigWrapper implements JSONConfig, Cloneable {
    ...
    protected <T> T getOfKnownType(@NotNull final String path,
        ...
        if (values.get(closestPath) instanceof Map && !path.equals(closestPath)) {
            com.willfp.eco.internal.config.json.JSONConfigSection section = new com.willfp.eco.internal.config.json.JSONConfigSection((Map<String, Object>) values.get(closestPath));
    ...
    protected Set<String> getDeepKeys(@NotNull final Set<String> list,
        ...
        for (String key : values.keySet()) {
            ...
            if (values.get(key) instanceof Map) {
                com.willfp.eco.internal.config.json.JSONConfigSection section = new com.willfp.eco.internal.config.json.JSONConfigSection((Map<String, Object>) values.get(key));
    ...
    public Config getSubsectionOrNull(@NotNull final String path) {
        if (values.containsKey(path)) {
            ...
            return new com.willfp.eco.internal.config.json.JSONConfigSection(subsection);
        } else {
            return null;
        }
    }
    ...
    public List<JSONConfig> getSubsectionsOrNull(@NotNull final String path) {
        ...
        for (Map<String, Object> map : maps) {
            configs.add(new com.willfp.eco.internal.config.json.JSONConfigSection(map));
        }
